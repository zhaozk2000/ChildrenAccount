...
//        sql.append("select * from " + DBHelper.tableNameTransaction + " where " + DBHelper.TransactionTable.holder + "=" + childId);
 ...
 */
    ...
    public static void executeAutomations(int lastExecuteDate, int today) {
        ...
        while(date<=today) {
            ...
            while(iter.hasNext()) {
                ...
                executeAutomation(automation, date, true, db);
            }
    ...
    private static void executeAutomation(Automation automation, int date, boolean dateCheck, SQLiteDatabase db) {
        if(dateCheck) {
            //  若不符合执行日期，则返回
    ...
    public static void executeAutomation(Automation automation, int date) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        executeAutomation(automation, date, false, db);
        db.close();
    }
