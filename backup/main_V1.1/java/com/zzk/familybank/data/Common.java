...
public class Common {
    ...
    public static int COLOR_DECREASE = Color.rgb(0, 100, 0);

    public static final String HOLDER_STRING = "账户持有人";
    ...
    public static long getBeginTimeOfYear(int year) {
        Calendar calendar = GregorianCalendar.getInstance();
        calendar.set(year, 0, 1, 0, 0, 0);
        return calendar.getTime().getTime();
    }

    public static long getBeginTimeOfYearMonth(int year, int month) {
        if(month>=12) {
            year += month / 12;
            month %= 12;
        }
