...
*/
    ...
    private static void executeAutomation(Automation automation, int date, SQLiteDatabase db) {
        ...
        if(automation.type == 1) {  //  结息，只在利息存入账户生成一条Transaction
            ...
            int sign = interest>=0 ? 1 : -1;
            transaction.amount = interest * sign;       //  amount不管支出收入都取正值
            transaction.category = automation.account2Category;
            transaction.fromto = automation.account2FromTo;
            transaction.notes = "";
            insertTransaction(db, transaction, sign);   //  transaction的afterbalance以及account2的balance都会一起更新
            ...
            transaction.amount = automation.int_data1;
            transaction.category = automation.account1Category;
            transaction.fromto = automation.account1FromTo;
            transaction.notes = "";
            int sign = -1;
            insertTransaction(db, transaction, sign);   //  transaction的afterbalance以及account1的balance都会一起更新
