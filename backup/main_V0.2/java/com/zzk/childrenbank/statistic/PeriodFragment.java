...
import com.zzk.childrenbank.data.DBHelper;
...
public abstract class PeriodFragment extends Fragment {
    ...
    public PeriodFragment(Child child) {
        this.child = child;
    }
    ...
    void queryData(Date begin, Date end, int accountIndex) {
        int accountId = -1;
        if(accountIndex>0) {
            accountId = child.accountList.get(accountIndex-1).id;
        }
        transactions = DAO.queryTransaction(child.id, accountId, begin, end, DBHelper.TransactionTable.date, false);
    }
