...
public class DAO {
    ...
    public static void loadDataFromDB(DataModel model){
        ...
        // 读取holder表
        model.holderList.clear();
        String sql = "select * from " + DBHelper.tableNameHolder;
        Cursor cursor = db.rawQuery(sql, null);
        int idIndex = cursor.getColumnIndex(DBHelper.HolderTable._id);
        int nameIndex = cursor.getColumnIndex(DBHelper.HolderTable.name);
        int createTimeIndex = cursor.getColumnIndex(DBHelper.HolderTable.create_time);
        while(cursor.moveToNext()) {
            Holder holder = new Holder();
            holder.id = cursor.getInt(idIndex);
            holder.name = cursor.getString(nameIndex);
            holder.createTime = new Date(cursor.getLong(createTimeIndex));
            model.holderList.add(holder);
        }
        cursor.close();
        ...
        int holderIndex = cursor.getColumnIndex(DBHelper.AccountTable.holder);
        ...
        while(cursor.moveToNext()) {
            ...
            account.holder = cursor.getInt(holderIndex);
            account.name = cursor.getString(nameIndex);
            account.balance = cursor.getInt(balanceIndex);
            Holder holder = model.findHolderById(account.holder);
            if(holder !=null) holder.accountList.add(account);
        }
        cursor.close();
        ...
        holderIndex = cursor.getColumnIndex(DBHelper.CategoryTable.holder);
        ...
        while(cursor.moveToNext()) {
            ...
            category.holder = cursor.getInt(holderIndex);
            ...
            Holder holder = model.findHolderById(category.holder);
            if(holder ==null) continue;
            Account account = holder.findAccountById(category.account);
            if(account!=null) {
        ...
        Iterator<Holder> iter = model.holderList.iterator();
        while(iter.hasNext()) {
            ...
            sql = "select * from " + DBHelper.tableNameTransaction + " where " + DBHelper.TransactionTable.holder + "=" + holder.id
                    + " order by " + DBHelper.TransactionTable.date + " desc limit " + Holder.TRANSACTION_BUFFER_SIZE;
            cursor = db.rawQuery(sql, null);
            idIndex = cursor.getColumnIndex(DBHelper.TransactionTable._id);
            holderIndex = cursor.getColumnIndex(DBHelper.TransactionTable.holder);
            ...
            while(cursor.moveToNext()) {
                ...
                transaction.holder = cursor.getInt(holderIndex);
                ...
                holder.transactionList.add(transaction);
            }
            cursor.close();
        }
    ...
    public static boolean insertHolder(Holder holder) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(DBHelper.HolderTable.name, holder.name);
        values.put(DBHelper.HolderTable.create_time, holder.createTime.getTime());
        long id = db.insert(DBHelper.tableNameHolder, null, values);
        db.close();
    ...
    public static boolean deleteHolder(String name) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        String whereClause = DBHelper.HolderTable.name + "='" + name + "'";
        int result = db.delete(DBHelper.tableNameHolder, whereClause, null);
        db.close();
        if(result==1) return true;
        return false;
    }

    public static boolean updateHolder(int id, String name) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(DBHelper.HolderTable.name, name);
        String whereClause = DBHelper.HolderTable._id + "=" + id;
        int result = db.update(DBHelper.tableNameHolder, values, whereClause, null);
        db.close();
        if(result==1) return true;
        return false;
    }

    public static boolean deleteAllHolder() {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        int result = db.delete(DBHelper.tableNameHolder, null, null);
    ...
    public static boolean insertAccount(Account account) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(DBHelper.AccountTable.holder, account.holder);
    ...
    public static boolean deleteAccount(int childId, String name) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        String whereClause = DBHelper.AccountTable.holder + "=" + childId + " AND " + DBHelper.AccountTable.name + "='" + name + "'";
    ...
    public static boolean deleteAllAccounts(int childId) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        String whereClause = DBHelper.AccountTable.holder + "=" + childId;
        int result = db.delete(DBHelper.tableNameAccount, whereClause, null);
        db.close();
        return true;
    }
    ...
    public static boolean insertCategory(Category category) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(DBHelper.CategoryTable.holder, category.holder);
...
//    public static int insertTransaction(Transaction transaction, int sign) {
    ...
    public static int insertTransaction(SQLiteDatabase db, Transaction transaction) {
        ...
        //  交易表中插入交易记录
        ContentValues values = new ContentValues();
        values.put(DBHelper.TransactionTable.holder, transaction.holder);
    ...
    public static int getPrevBalance(SQLiteDatabase db, Transaction transaction){
        ...
        String sql = "select * from " + DBHelper.tableNameTransaction +
                " where " + DBHelper.TransactionTable.holder + "=" + transaction.holder +
    ...
    public static int getPrevBalance(Transaction transaction) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        int balance = getPrevBalance(db, transaction);
        return balance;
    }
...
//    public static int deleteTransaction(Transaction transaction, int sign) {
    ...
    public static int modifyTransactionData(Transaction transaction) {
        int result = 1;
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        try{
            // 更新交易表中本条交易记录
    ...
    public static List<Transaction> queryTransaction(int childId, int accountId, Date begin, Date end, String orderBy, boolean asc) {
        ...
        sql.append("where " + DBHelper.TransactionTable.holder + "=" + childId);
        if(accountId>=0)
        ...
        return queryTransaction(sql.toString());
    }

    public static List<Transaction> queryTransaction(String where) {
        StringBuffer sql = new StringBuffer();
        sql.append("select * from ").append(DBHelper.tableNameTransaction);
        if(where.length()>0) sql.append(" ").append(where);

        List<Transaction> result = new ArrayList<>();
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery(sql.toString(), null);
        int idIndex = cursor.getColumnIndex(DBHelper.TransactionTable._id);
        int childIndex = cursor.getColumnIndex(DBHelper.TransactionTable.holder);
        ...
        while(cursor.moveToNext()) {
            Transaction transaction = new Transaction();
            transaction.id = cursor.getInt(idIndex);
            transaction.holder = cursor.getInt(childIndex);
            transaction.account = cursor.getInt(accountIndex);
            transaction.category = cursor.getInt(categoryIndex);
            transaction.amount = cursor.getInt(amountIndex);
            transaction.afterBalance = cursor.getInt(afterBalanceIndex);
    ...
    public static void executeAutomations(int lastExecuteDate, int today) {
        ...
        while(date<=today) {
            ...
            while(iter.hasNext()) {
                ...
                boolean executed = executeAutomation(automation, date, seconds, true, db);
                if(executed) seconds ++;
            }
    ...
    public static void testAllAutomations() {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        List<Automation> automationList = new ArrayList<>();
        loadAutomationFromTable(db, automationList);

        int date = IntDate.getToday();
        int seconds = 1;

            Iterator<Automation> iter = automationList.iterator();
            while(iter.hasNext()) {
                Automation automation = iter.next();
                boolean executed = executeAutomation(automation, date, seconds, false, db);
                if(executed) seconds ++;
            }
    ...
    private static boolean executeAutomation(Automation automation, int date, int seconds, boolean dateCheck, SQLiteDatabase db) {
        if(dateCheck) {
            //  若不符合执行日期，则返回
            if(automation.period ==0) {   //  按年
                if(date % 10000 != automation.date) return false;
            } else if(automation.period ==1)  {   //  按月
                if(date % 100 != automation.date) return false;
            } else {
                return false;
            }
        }
        ...
        transaction.date = new Date(transaction.date.getTime() + seconds*1000); // 让每个交易时间差1秒，区别交易顺序
        // 符合执行日期，则执行
        if(automation.type == 1) {  //  结息，只在利息存入账户生成一条Transaction
            ...
            transaction.holder = account2.holder;
            ...
            //  取款交易
            transaction.holder = account1.holder;
            ...
            transaction.fromto = automation.account2FromTo;
            transaction.notes = "";
            transaction.date = new Date(transaction.date.getTime() + 500); // 让存取交易时间差500毫秒，区别交易顺序
            insertTransaction(db, transaction);   //  transaction的afterbalance以及account2的balance都会一起更新
        }
        return true;
    }

    public static void executeAutomation(Automation automation, int date) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        executeAutomation(automation, date, 1, false, db);
        db.close();
    }
